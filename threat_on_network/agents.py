...
class AgenteActivoTI(mesa.Agent):
    ...
    def try_to_infect_neighbors(self):
        nodos_vecinos = self.model.grid.get_neighbors(
        ...
        for a in candidato_infeccion:
            if type(a) is AgenteComputo:
                print("Eficacia de antivirus del agente de computo",
                      a.eficacia_antivirus)
    ...
    def step(self):
        if self.estado is INFECCION.INFECTADO or self.estado is INFECCION.IMPACTADO:
            self.try_to_infect_neighbors()
            if self.model.tipo_activo_objetivo == self.tipo:
                self.estado = INFECCION.IMPACTADO
...
class AgenteComputo(AgenteActivoTI):
    ...
    def try_gain_resistance(self):
        if self.random.random() < self.model.probabilidad_ganar_resistencia:
    ...
    def step(self):
        if self.estado is INFECCION.INFECTADO or self.estado is INFECCION.IMPACTADO:
            self.try_to_infect_neighbors()
            if self.model.tipo_activo_objetivo == "computo":
                self.estado = INFECCION.IMPACTADO
                self.confidencialidad = 1-self.model.impacto_confidencialidad
                self.integridad = 1-self.model.impacto_integridad
                self.disponibilidad = 1-self.model.impacto_disponibilidad
...
class AgenteInformacion(mesa.Agent):
    ...
    def step(self):
        print("STEP info")
        if self.estado is INFECCION.INFECTADO or self.estado is INFECCION.IMPACTADO:
            print("tipo_activo_objetivo", self.model.tipo_activo_objetivo)
            if self.model.tipo_activo_objetivo == "informacion":
                self.estado = INFECCION.IMPACTADO
                self.confidencialidad = 1-self.model.impacto_confidencialidad
                self.integridad = 1-self.model.impacto_integridad
                self.disponibilidad = 1-self.model.impacto_disponibilidad
...
class AgenteProceso(mesa.Agent):
    ...
    def get_produccion(self):
        ...
        if len(activos_dependencia) == 0:
            ...
            return sum(activos_dependencia) / len(activos_dependencia)
